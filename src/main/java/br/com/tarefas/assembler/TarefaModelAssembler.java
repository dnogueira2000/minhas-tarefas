...
public class TarefaModelAssembler implements RepresentationModelAssembler<Tarefa, EntityModel<TarefaResponse>> {
	...
	public EntityModel<TarefaResponse> toModel(Tarefa tarefa) {
		...
		EntityModel<TarefaResponse> tarefaModel = EntityModel.of(tarefaResponse,
				WebMvcLinkBuilder.linkTo(WebMvcLinkBuilder.methodOn(TarefaController.class).listarPorId(tarefaResponse.getId())).withSelfRel(),
				WebMvcLinkBuilder.linkTo(WebMvcLinkBuilder.methodOn(TarefaController.class).todasTarefas(new HashMap<>())).withRel("tarefas"),
				WebMvcLinkBuilder.linkTo(WebMvcLinkBuilder.methodOn(TarefaCategoriaController.class).listarPorId(tarefaResponse.getCategoriaId())).withRel("categoria"),
				WebMvcLinkBuilder.linkTo(WebMvcLinkBuilder.methodOn(UsuarioController.class).listarPorId(tarefaResponse.getUsuarioId())).withRel("usuario")
				);
